    ...
    subItems: [
      {
        ...
        href: `/Backend/Buildchem/Account/Profile${userId ? `?id=${encodeURIComponent(userId)}` : ""}`
      },
    ],
  };
    ...
    <div className="flex flex-col items-center flex-grow overflow-y-auto text-xs p-2">
      <div className="w-full">
        <button
          ...
          className={`flex items-center w-full p-4 hover:bg-white rounded hover:text-blue-900 transition-all duration-300 ease-in-out hover:shadow-md active:scale-95 ${collapsed ? "justify-center" : ""
            }`}
        >
            ...
            <div>
              {myProfileItem.subItems.map((subItem, subIndex) => (
                <Link
                  key={subIndex}
                  href={subItem.href}
                  className="flex items-center w-full p-4 bg-gray-200 hover:bg-white hover:text-blue-900 transition-all duration-300 ease-in-out"
                >
      ...
      <div className="w-full mt-1">
        <Link
          href={`/Backend/Buildchem/Dashboard/${userId ? `?id=${encodeURIComponent(userId)}` : ""}`}
          className="flex items-center w-full p-4 bg-white hover:bg-white hover:text-blue-900 text-blue-900 rounded-md transition-all duration-300 ease-in-out hover:shadow-md active:scale-95"
        >
          <span className="mr-1">
        ...
        .map((item, index) => (
          <div key={index} className="w-full">
            <button
              ...
              className={`flex items-center w-full p-4 rounded hover:bg-white hover:text-blue-900 transition-all duration-300 ease-in-out hover:shadow-md active:scale-95 ${collapsed ? "justify-center" : ""
                }`}
            >
              <item.icon size={18} />
            ...
            >
              {item.subItems.map((subItem, subIndex) => (
                <div key={subIndex} className="w-full">
                  <Link
                    href={subItem.href}
                    className="flex flex-col mb-1 p-4 bg-gray-200 hover:bg-white hover:text-blue-900 transition-all duration-300 ease-in-out rounded"
                  >
